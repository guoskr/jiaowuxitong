...
public class searchcourse extends Activity {
    ...
    private void initView() {
        ...
        switch (currentSearchType) {
            case TYPE_SEARCH_NUMBER:
                etSearch.setHint("请输入你要搜索的课程号");
                break;
            case TYPE_SEARCH_NAME:
                etSearch.setHint("请输入你要搜索的课程名");
                break;
        ...
        rvSearch.setLayoutManager(new LinearLayoutManager(searchcourse.this));
    ...
    private void refreshRecyclerView(String s) {
        ...
        switch (currentSearchType) {
            case TYPE_SEARCH_NAME:
                mCursor = mSQLiteDatabase.query("course", null, "cname like ?", new String[]{"%" + s + "%"}, null, null, null);
                break;
            case TYPE_SEARCH_sdept:
                mCursor = mSQLiteDatabase.query("course", null, "sdept like ?", new String[]{"%" + s + "%"}, null, null, null);
                break;
            default:
                mCursor = mSQLiteDatabase.query("course", null, "cno like ?", new String[]{"%" + s + "%"}, null, null, null);
        }
        ...
        while (true) {
            if (size-- == 0)
                break;
            mCursor.moveToNext();
            sno.add(mCursor.getString(mCursor.getColumnIndex("cno")));
            sname.add(mCursor.getString(mCursor.getColumnIndex("cname")));
            sdept.add(mCursor.getString(mCursor.getColumnIndex("credit")));
        }
        mCursor.close();
        rvSearch.setAdapter(new searchadapter(searchcourse.this, sno,sname,sdept));
    }
